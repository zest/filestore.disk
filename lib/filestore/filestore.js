...
module.exports.export = function (dirPath) {
    var tgz = new targz(),
        zipStore = path.join(os.tmpdir(), 'export-' + parseInt(Math.random()*999999) + 'tar.gz');
...
module.exports.import = function (dirPath, readStream) {
    var tgz = new targz(),
        zipStore = path.join(os.tmpdir(), 'import-' + parseInt(Math.random()*999999) + 'tar.gz'),
...
module.exports.watch = function (baseDir, globPattern, eventType, callback) {
    ...
    if(!eventType || eventType === 'create') {
        config.fileEvent.on('create', function (filePath) {
            if(minimatch(filePath, globPattern)) {
                callback(filePath, 'create');
            }
        });
    }
    if(!eventType || eventType === 'remove') {
        config.fileEvent.on('remove', function (filePath) {
            if(minimatch(filePath, globPattern)) {
                callback(filePath, 'remove');
            }
        });
    }
};
